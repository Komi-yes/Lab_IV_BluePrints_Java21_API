...
Comando con el que se construyeron los archivos base de Dockerfile, compose.yaml y .dockerignore. A partir de estos archivos y la configuración básica que ofrece Docker Compose, se realizaron cambios para poder crear y correr una base de datos en PostgreSQL. Para que la base de datos se pudiera conectar con nuestra API, se reservó un puerto para que esta pudiera correr: se usó el puerto *5432* y el servicio se dejó con el puerto que ya venía usando, el cual era el *8080*. Dentro de los cambios también se agregaron configuraciones básicas de seguridad y autenticación para la base de datos, así como la versión y el volumen que se iba a usar para la creación de la misma. Debido a que esto fue una prueba y no para ningún tipo de producción, no se declararon secrets para las credenciales o contraseñas y se usaron valores por defecto quemados dentro de las mismas configuraciones.

Una vez configurada y "creada" la base de datos, se realizó la implementación real de la persistencia mediante la clase src/main/java/edu/eci/arsw/blueprints/persistence/impl/PostgresBlueprintPersistence.java, usando los contratos ya anteriormente creados por src/main/java/edu/eci/arsw/blueprints/persistence/impl/BlueprintPersistence.java, la cual reemplaza la anterior versión de memoria en ejecución. Para conectar directamente a la base de datos, se usó la interfaz src/main/java/edu/eci/arsw/blueprints/persistence/impl/JpaBlueprintRepository.java, que extiende de JpaRepository, la cual también se agregó como dependencia del proyecto dentro del pom.xml y permite la fácil comunicación con nuestra base de datos dockerizada.

No se crearon datos por defecto debido a que, como la persistencia se mantiene entre ejecuciones, no se vio necesaria la inserción de datos en cada construcción de la base de datos.

Cabe aclarar que la base de datos está dockerizada, por lo que, una vez acabe la ejecución, la base de datos permanecerá activa hasta que se pare la ejecución del contenedor. Si esto pasa, la información sí se verá comprometida y borrada.
